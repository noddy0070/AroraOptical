...
export default function BlueFilterLens({form,setForm,handleFocus,amount,setAmount,basePrice=0,setCoatingPrice}) {
    return (
        <div id='blueFilterLensMain' className='px-[5vw] md:px-0 py-[6vw] md:py-0'>
            ...
            <div className='mx-auto py-[6vw] md:py-[1.5vw] px-[5vw] md:px-[1vw] flex flex-row gap-[6vw] md:gap-[1.5vw] w-full md:w-min overflow-x-auto hide-scrollbar md:overflow-visible'>
                    ...
                    }} title={"Green Coat"} description={"Reduces glare and enhances clarity while blocking harmful blue light."} price={form.lensType=='Bifocal'?1950:1090} />
                    <LensFeatureBox onclick={()=>{
                        setForm({...form,lensCoating:"Blue-Filter/Blue"});
                        const price = form.lensType=='Bifocal'?1950:1090;
                        setAmount(amount+price);
                        if(setCoatingPrice) setCoatingPrice(price);
                        handleFocus("lensThickness");
                    }} title={"Blue Coat"} description={"Specifically designed to filter blue light, offering better eye comfort during screen use."} price={form.lensType=='Bifocal'?1950:1090} />
                    <LensFeatureBox onclick={()=>{
                        setForm({...form,lensCoating:"Blue-Filter/Mixed"});
                        const price = form.lensType=='Bifocal'?1950:1090;
                        setAmount(amount+price);
                        if(setCoatingPrice) setCoatingPrice(price);
                        handleFocus("lensThickness");
                    }} title={"Mixed Coat"} description={"A balanced combination of blue and green coatings for superior protection and clarity."} price={form.lensType=='Bifocal'?1950:1090} />
            </div>
